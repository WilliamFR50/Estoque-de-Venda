...
// Função para renderizar os produtos na tela (visão por categoria)
function renderCategorizedView(products) {
    ...
    sortedCategories.forEach(category => {
        ...
        // Ordenar produtos da categoria alfabeticamente
        const sortedProducts = sortProducts(groupedProducts[category], 'name');
...
// Função para renderizar os produtos na tela (visão em lista)
function renderListView(products) {
    ...
    // Ordenar produtos
    const sortedProducts = sortProducts(products, document.getElementById('sort-select').value);
...
// Função para alternar entre modos de usuário
function toggleUserView() {
    isOwnerView = !isOwnerView;
    
    // Atualizar a interface conforme o modo
    document.getElementById('user-type').textContent = `Modo: ${isOwnerView ? 'Dono' : 'Funcionário'}`;
    document.getElementById('profit-section').style.display = isOwnerView ? 'block' : 'none';
    
    // Atualizar o valor do lucro se for o dono
    if (isOwnerView) {
        const profit = calculateTotalProfit();
        document.getElementById('profit-value').textContent = `R$ ${profit.toFixed(2)}`;
    }
}
